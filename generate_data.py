...
# ── 11. Write JSON file ──────────────────────────────────────────────────────
# Emitted as a static asset (public/data.json) and fetched at runtime by
# src/data/data.js, so the ~1.2 MB payload stays out of the JS bundle.
dataset = {
    ...
    "kpis": kpis,
    "faturamentoPorPartida": faturamentoPorPartida,
    "publicoPorTorcedor": publicoPorTorcedor,
    "publicoETicketPorTime": publicoETicketPorTime,
    "faturamentoPorCampeonatoAno": faturamentoPorCampeonatoAno,
    "faturamentoPorMes": faturamentoPorMes,
    "faturamentoPorSetor": faturamentoPorSetor,
    "unitarioPorTimeESetor": unitarioPorTimeESetor,
    "faturamentoPorAdversario": faturamentoPorAdversario,
    "noShowAnalysis": noShowAnalysis,
    "noShowPorTorcedor": noShowPorTorcedor,
    "precosPorTimeETorcedor": precosPorTimeETorcedor,
    "publicoPorSetorPartida": publicoPorSetorPartida,
    "torcedorCols": torcedorCols,
    "allSetorNames": all_setor_names,
    "plPorPartida": plPorPartida,
    # Convenience lists
    "CAMPEONATOS": sorted(set(p["campeonato"] for p in partidas if p["campeonato"])),
    "SETORES": sorted(set(b["idSetor"] for b in bordero)),
}
