...
function CustomXTick({ x, y, payload }) {
  const [timeName, rodada, campeonato] = payload.value.split('|');
  const logoFile = LOGO_MAP[timeName];
  const compLogo = COMP_LOGOS[campeonato];
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      {/* team logo */}
      {logoFile ? (
        <image href={`/logos/${logoFile}`} x={-TEAM_SIZE/2} y={4} width={TEAM_SIZE} height={TEAM_SIZE} style={{objectFit:'contain'}} />
      ) : (
      ...
      {compLogo && (
        <image href={`/logos/${compLogo}`} x={-COMP_SIZE/2} y={4 + TEAM_SIZE + 2} width={COMP_SIZE} height={COMP_SIZE} />
      )}
      <text x={0} y={4 + TEAM_SIZE + COMP_SIZE + 8} textAnchor="middle" fill={C.t3} fontSize={7}>{rodada}</text>
    </g>
  );
}
